...
void Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims){
  ...
  I2 = (uint8_t*)_mm_malloc(bpl*height*sizeof(uint8_t),16);       //for input
  memset (I1,0,bpl*height*sizeof(uint8_t));
  memset (I2,0,bpl*height*sizeof(uint8_t));                     //set memory for input image
  if (bpl==dims[2]) {
  ...
  Descriptor desc2(I2,width,height,bpl,param.subsampling);                  //用desc类型记录下图片，内存指针的开头给I_desc
...
vector<Elas::support_pt> Elas::computeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc) {
  ...
  int16_t* D_can = (int16_t*)calloc(D_can_width*D_can_height,sizeof(int16_t));  //init D_can zero array for 为选取的差值点初始化一个零数组
  ...
  for (int32_t u_can=1; u_can<D_can_width; u_can++) {
    ...
    for (int32_t v_can=1; v_can<D_can_height; v_can++) {
      ...
      *(D_can+getAddressOffsetImage(u_can,v_can,D_can_width)) = -1;      //D_can + ( u_can*D_can_width + v_can ) = -1
      
      // find forwards
      d = computeMatchingDisparity(u,v,I1_desc,I2_desc,false);          //在一个3X3的范围内计算disparity
      if (d>=0) {
